...
coot::util::multi_parse_prosmart_log_and_gen_CO_plot() {
   std::string dir = "pdb";
   std::vector<std::string> sub_dirs = util::gather_files_by_patterns(dir, { "*" }, GATHER_DIRECTORIES | GATHER_LINKS);
   for (unsigned int i=0; i<sub_dirs.size(); i++) {
      const std::string &sub_dir = sub_dirs[i];
      std::vector<std::string> sub_dir_files = util::gather_files_by_patterns(sub_dir, { "*.pdb" });
      for (unsigned int j=0; j<sub_dir_files.size(); j++) {
         ...
         std::vector<std::string> chain_files = util::gather_files_by_patterns(prosmart_1, { code_star });

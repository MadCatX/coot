...
coot::daca::read_tables(const std::string &dir) {
   ...
   std::vector<std::string> files = util::gather_files_by_patterns(dir, { "*.table" });
...
coot::daca::write_tables_using_reference_structures_from_dir(const std::string &dir_name,
   ...
   geom.init_standard();
   std::vector<std::string> files = util::gather_files_by_patterns(dir_name, { "*.pdb" });
...
coot::daca::score_molecule(const std::string &pdb_file_name) {
   ...
   int score = 0;
   if (util::file_exists(pdb_file_name)) {
